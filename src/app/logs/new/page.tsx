    ...
    <DashboardLayout>
      <div className="max-w-4xl mx-auto">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 pb-20">
          ...
          <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-8">
            <div>
              <div className="flex items-center gap-2 text-xs font-bold text-blue-600 uppercase tracking-widest mb-1">
                <ClipboardCheck size={14} /> Protocol Initialization
              </div>
              <h1 className="text-3xl font-bold tracking-tight text-slate-900">New Service Entry</h1>
              <p className="text-slate-500 font-medium mt-1">Initialize a new field intervention record.</p>
            </div>
          </header>
                ...
                {jobType === "ict" ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="number" {...register("ictData.signalStrength")} placeholder="Measure and enter value" className="w-full h-11 px-4 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:bg-white transition-all font-bold text-slate-600" />
                    </div>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input type="number" {...register("solarData.systemVoltage")} placeholder="Enter nominal voltage" className="w-full h-11 px-4 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:bg-white transition-all font-bold text-slate-600" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[11px] font-black uppercase tracking-widest text-slate-500">Battery Health (SOH %)</label>
                      <input type="number" {...register("solarData.batteryHealth")} placeholder="Estimated health percentage" className="w-full h-11 px-4 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:bg-white transition-all font-bold text-slate-600" />
                    </div>
                  </div>
                )}
              </div>
          ...
          {/* Sticky Action Footer for Mobile, and desktop visual grouping */}
